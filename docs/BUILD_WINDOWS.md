...
The output folder must not exist. Build logs, a frozen app folder, setup EXE, component inventory and checksum are produced. `Ariadne.exe --self-test NEW_TEST_FOLDER` runs a bundled Tk workflow: creates transparent artwork with a hole and two colors, converts through the real GUI worker, saves PES/DST, decodes the proof and records the result. No installed Python or network access is used by that test.
